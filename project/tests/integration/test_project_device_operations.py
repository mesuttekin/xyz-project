...
from project.tests.integration.operation_helper import add_project, add_device
...
class TestUserController(BaseTestCase):
    ...
    def test_givenDeviceData_whenCallPost_thenAddTheDevice(self):
        """ Test for add device """
        with self.client:
            response = add_project_device(self)
            data = json.loads(response.data.decode())
    ...
    def test_givenDeviceId_whenCallGet_thenGetTheDevice(self):
        ...
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(device_id, data['device_id'])
            self.assertTrue(response.content_type == 'application/json')
